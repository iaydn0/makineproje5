...
st.title("MPG Tahmini - Regresyon ve Sınıflandırma")
...
model_files = {
    ...
    "Naive Bayes (Sınıflandırma)": "naive_bayes_model.pkl"
}
...
if st.button("Tahmin Et"):
    try:
        ...
        if model_name == "Naive Bayes (Sınıflandırma)":
            st.success(f"MPG Sınıfı: {prediction[0]}")
        else:
